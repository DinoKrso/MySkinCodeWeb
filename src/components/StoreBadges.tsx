import { APP_STORE_URL, GOOGLE_PLAY_URL } from "../content/download";
import "./StoreBadges.css";

function AppleStoreIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
    </svg>
  );
}

function GooglePlayIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#EA4335"
        d="M3 20.5V3.5C3 2.91 3.34 2.39 3.84 2.15L13.69 12 3.84 21.85C3.34 21.6 3 21.09 3 20.5Z"
      />
      <path
        fill="#FBBC04"
        d="M16.81 15.12 6.05 21.34 14.54 12.85l2.27 2.27Z"
      />
      <path
        fill="#4285F4"
        d="M20.16 10.81c.34.27.59.69.59 1.19s-.22.83-.57 1.1l-2.29 1.32-2.5-2.5 2.5-2.5 2.27 1.31Z"
      />
      <path
        fill="#34A853"
        d="M6.05 2.66 16.81 8.88 14.54 11.15 6.05 2.66Z"
      />
    </svg>
  );
}

type StoreBadgeProps = {
  store: "apple" | "google";
  large?: boolean;
};

function StoreBadge({ store, large = false }: StoreBadgeProps) {
  const isApple = store === "apple";
  const label = isApple ? "Preuzmi na App Store" : "Preuzmi na Google Play";
  const href = isApple ? APP_STORE_URL : GOOGLE_PLAY_URL;

  if (!href) {
    return (
      <span
        className={`store-badge store-badge--disabled${large ? " store-badge--lg" : ""}`}
        aria-label={`${label} — uskoro dostupno`}
      >
        {isApple ? <AppleStoreIcon /> : <GooglePlayIcon />}
        <span className="store-badge__label">{label}</span>
      </span>
    );
  }

  return (
    <a
      href={href}
      target="_blank"
      rel="noreferrer"
      className={`store-badge${large ? " store-badge--lg" : ""}`}
      aria-label={label}
    >
      {isApple ? <AppleStoreIcon /> : <GooglePlayIcon />}
      <span className="store-badge__label">{label}</span>
    </a>
  );
}

type Props = {
  large?: boolean;
  className?: string;
};

export default function StoreBadges({ large = false, className }: Props) {
  return (
    <div className={`store-badges${className ? ` ${className}` : ""}`}>
      <StoreBadge store="apple" large={large} />
      <StoreBadge store="google" large={large} />
    </div>
  );
}
